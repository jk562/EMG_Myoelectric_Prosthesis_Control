"""
Streamlit frontend for EMG -> hand kinematics -- covers BOTH pipelines in
one app, switchable from a sidebar selector:

  .mat/.csv/.npy  -- quick_emg_to_kinematics.py: Ninapro DB2, 22-channel
                     CyberGlove kinematics, best-effort anatomical joint
                     names, 3D fingertip-workspace visual.
  .fif            -- rawtest_emg_to_kinematics.py: RAW TEST recordings,
                     15 generic "Angle" channels (no verified anatomical
                     mapping for this device, so no bone names/workspace).
...
Shows, for an uploaded/selected EMG recording:
  ...
  3. the action being performed, if the file has a label -- .mat via the
     Ninapro `restimulus` field, .fif via MNE annotations (add discrete
     action/gesture segments to a RAW TEST recording with
     `raw.set_annotations(...)` + `raw.save(...)` and they'll drive the
     same action-selector UI). Falls back to a plain window-index slider
     only when no labels are found at all.
  4. a button to calibrate the model to this specific subject for a
     realistic, honest deployment-accuracy number (the base model trains
     on every available subject, so cold accuracy reflects fit to
     already-seen data, not generalisation to a new person)

Run:
    streamlit run EMG_frontend.py

Requires EMG-KinNet.pt and/or RawTest-KinNet.pt depending on which mode you
use. Reuses each backend script's model, loader, metrics, and fine-tuning
directly instead of keeping copies, so anything the CLI does works here too
and stays in sync.
"""
...
import quick_emg_to_kinematics as qmat
import rawtest_emg_to_kinematics as qfif

RAWTEST_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "RAW TEST")

# Each mode bundles everything the shared UI code needs to stay backend-
# agnostic: which module to call predict/finetune/Net on, which file types
# and folder to browse, which checkpoint files to list, and whether the
# CyberGlove-specific anatomy sections (action label, bone names, 3D
# workspace) apply -- they don't for the RAW TEST device, which has no
# verified joint-name mapping for its 15 "Angle" channels.
MODES = {
    ".mat / .csv / .npy  (Ninapro DB2, 22-joint CyberGlove)": {
        "backend": qmat,
        "ext": ["mat", "csv", "npy"],
        "data_dir": "data",
        "model_glob": "EMG-KinNet*.pt",
        "has_anatomy": True,
    },
...
# below. If you have the original paper's movement figure (Atzori et al.
# 2014, Fig. 2) you could read the individual names off it and this can be
# extended to name them specifically instead.
ACTION_NAMES = {
    0: "Rest",
    1: "Thumb up",
...
# for any specific subject or prosthetic device. Only used to give the 3D
# workspace plot a physically-plausible scale; treat the mm axes there as
...
def fingertip_xyz(norm, finger):
    ...
    0-1-normalised 22-channel vector at one timestep. Reuses the same
    MCP/PIP/DIP flexion chain as elsewhere, plus that finger's abduction
    channel for lateral spread, extended by FINGER_SEGMENT_MM's illustrative
    segment lengths. Returns (x, y, z) in mm."""
...
@st.cache_resource
def load_kinematics_model(path, _backend):
    """_backend is the mode's module (quick_emg_to_kinematics or
    rawtest_emg_to_kinematics) -- underscore prefix tells st.cache_resource
    not to try hashing it (modules aren't hashable the way it wants), the
    path string is what actually keys the cache."""
    if not os.path.exists(path):
        return None, None
    ck = torch.load(path, map_location=_backend.dev, weights_only=False)
    net = _backend.Net().to(_backend.dev)
...
def fif_action_labels(path, n_windows, win, hop, fs):
    ...
    present -- lets you add discrete gesture/action segments to a RAW TEST
    recording (e.g. via raw.set_annotations(...) + raw.save(...)) and have
    them drive the same action-selector UI as the .mat pipeline's
    restimulus labels, instead of a plain window-index slider. Returns None
    if the file has no annotations covering any window (the default single
    "model trained" annotation some of these files ship with doesn't count
    as a real action label)."""
    import mne
...
def windows_and_labels(path, backend):
    """Same loader+windowing each backend's own predict() uses, plus the
    recorded movement label if this file has one (.mat via `restimulus`,
    .fif via MNE annotations, see fif_action_labels) -- that part is UI-only,
    not part of the prediction."""
    load_fn = backend.load_emg_file if hasattr(backend, "load_emg_file") else backend.load_fif_file
    emg, glove, channel_mask, warns = load_fn(path)
    ...
    if path.lower().endswith(".mat"):
        ...
        labels = fif_action_labels(path, len(X), backend.WIN, backend.HOP, backend.FS)

    return X, Y, M, labels, emg[:, :n_real], warns


def predict(net, ck, X, M, backend):
    """Runs the file's windows through Net in batches -- matches the
    backend's own predict(). Returns predictions already rescaled back to
    real units (xm/xs/ym/ys applied internally).

    has_sim: only quick_emg_to_kinematics.py has the cross-channel
    cosine-similarity branch (channel_cosine_similarity, Net taking
    (x, feat, sim, mask)) -- rawtest_emg_to_kinematics.py doesn't, so this
    branches on the backend rather than assuming both match."""
    xm, xs, ym, ys = ck["xm"], ck["xs"], ck["ym"], ck["ys"]
    fm, fs = ck["fm"], ck["fs"]
    has_sim = hasattr(backend, "channel_cosine_similarity")
    Xn = ((X - xm) / xs).astype(np.float32)
    Fn = ((backend.handcrafted_features(X, M) - fm) / fs).astype(np.float32)
    if has_sim:
        sm, ss = ck["sm"], ck["ss"]
        Sn = ((backend.channel_cosine_similarity(X, M) - sm) / ss).astype(np.float32)
    out = []
    with torch.no_grad():
        for i in range(0, len(Xn), backend.BATCH):
            xb = torch.from_numpy(Xn[i:i + backend.BATCH]).to(backend.dev)
            mb = torch.from_numpy(M[i:i + backend.BATCH].astype(np.float32)).to(backend.dev)
            fb = torch.from_numpy(Fn[i:i + backend.BATCH]).to(backend.dev)
            if has_sim:
                sb = torch.from_numpy(Sn[i:i + backend.BATCH]).to(backend.dev)
...
def normalize_channels(values, ref):
    ...
    to normalise each channel to roughly 0-1 -- keeps per-channel scale
    differences (e.g. the joint-10 outlier) from distorting anything built
    on top of this. Uses the 1st-99th percentile rather than raw min/max, so
    a handful of noisy outlier samples (glove sensor glitches etc.) can't
    single-handedly blow up the scale -- values beyond that range just clip
    to 0/1 instead of stretching everything else to accommodate them."""
...
def movement_blocks(labels, min_len=3):
    ...
    labeled 'trial' (a single real reach/movement) in the trajectory plot
    below, instead of every repetition of every movement in the file.
    Skips label 0 (rest) and runs shorter than min_len windows (label
    noise/transition blips, not a real movement)."""
    blocks, seen = [], set()
    i, n = 0, len(labels)
    while i < n:
        lab = int(labels[i])
...
st.caption(f"Predicts continuous kinematics from forearm EMG -- {mode_label}, "
          f"1-{backend.MAX_CHANNELS} channels, with or without ground truth")

# ---- model selection ----
st.sidebar.header("Model")
model_files = sorted(f for f in glob.glob(mode["model_glob"]) if "-SSL" not in os.path.basename(f))
if not model_files:
    st.error(f"No `{mode['model_glob']}` found. Train one first (see that backend's `train` command).")
    st.stop()
...
kin_net, kin_ck = load_kinematics_model(model_path, backend)
...
in_mode = st.sidebar.radio("Source", ["Upload file", f"Pick from {mode['data_dir']}/"], key=f"src_{mode_label}")
...
if in_mode == "Upload file":
    ...
    files = sorted([f for ext in mode["ext"] for f in glob.glob(os.path.join(mode["data_dir"], f"*.{ext}"))])
    if not files:
        st.sidebar.warning(f"No {'/'.join(mode['ext'])} files in {mode['data_dir']}/")
    else:
...
X, Y_true, M, labels, emg_raw, warns = windows_and_labels(path, backend)
...
pred = predict(kin_net, kin_ck, X, M, backend)
...
calib_options = sorted([f for ext in mode["ext"] for f in glob.glob(os.path.join(mode["data_dir"], f"*.{ext}"))])
calib_source = st.radio("Calibrate using", ["This file (currently loaded)", f"A different file from {mode['data_dir']}/"],
...
if calib_source.startswith("A different file"):
    ...
    st.caption(f"This file has no ground-truth kinematics -- pick \"A different file from {mode['data_dir']}/\" "
...
if calib_path == path and Y_true is None:
    ...
    with st.spinner("Calibrating..."):
        with contextlib.redirect_stdout(log):
            try:
                backend.finetune(calib_path, model_path)
...
def _action_name(a):
    ...
    the annotation description strings directly, shown as-is since there's
    no separate name mapping for them. For .mat: Exercise B (1-17) gets a
    specific name from ACTION_NAMES; Exercise C/D (18-49) get a category +
    within-exercise position, since no verified specific name list was
    found for those (see the ACTION_NAMES comment above) -- still real
    information (which exercise, which movement number in it), just not a
    specific grasp/force-pattern name."""
...
window_s = (t * backend.HOP) / backend.FS
...
if show_full:
    seg = emg_raw
    tt = np.arange(len(seg)) / backend.FS
...
if Y_true is not None:
    ...
    # whole-file accuracy number is partly inflated -- it includes the 70%
    # of windows the model was directly trained on during calibration, not
    # just the 30% held-out split calibration actually reported. Detect
    # that (checkpoint name matches "<prefix>_finetuned_<this file's stem>")
    # and recompute the SAME leakage-safe held-out split finetune() used
    # (same seed, same params -> identical split) so the number shown here
    # matches what Calibrate actually reported, not an optimistic blend.
    file_stem = os.path.splitext(os.path.basename(path))[0]
    ckpt_prefix = mode["model_glob"].replace("*.pt", "")
...
if has_anatomy:
    ...
    st.caption("3D fingertip path for a handful of labeled movements in this file -- drag to rotate. Each "
              "numbered/colored trial below is ONE real movement repetition (not a synthetic sweep): solid "
              "line = the true (recorded) path the hand actually took, dashed line in the same color = what "
              "the model predicted for that same movement, ● = start (rest, just before the movement "
              "begins), ◆ = target (where the true hand ended up), ✕ = endpoint (where the PREDICTED path "
              "ended up -- its distance from the ◆ is the model's positional error for that movement). "
              "**Built on best-effort assumptions**: the MCP/PIP/DIP channel mapping (unverified, see note "
              "above) *and* generic illustrative adult finger-segment lengths (not measured for this subject "
              "or any specific prosthetic design) -- treat the mm axes as approximate scale, the *shape and "
              "endpoint-error* comparison is the useful part.")
...
if Y_true is None:
    ...
    with st.spinner("Running all checkpoints..."):
        for mf in model_files:
            net_i, ck_i = load_kinematics_model(mf, backend)
            pred_i = predict(net_i, ck_i, X, M, backend)
