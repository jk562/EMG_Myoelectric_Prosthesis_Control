...
class WeightedMSE:
    """TRIED and REVERTED -- kept here only for reference, not used in the
    active training loop below (see main(), which uses plain nn.MSELoss
    for training now). Per-joint MSE weighted by an EMA of that joint's
    own recent loss -- joints the model is currently doing relatively
    worse on get upweighted, so they compete harder for the shared
    representation's capacity. Motivated by three separate Test8
    benchmark runs (baseline, single-head attention, transformer block)
    all showing the SAME joint (2) as the persistent worst performer,
    even though the training target is already per-joint z-score
    normalised (unit variance) -- the hypothesis was that normalisation
    only balances RAW-SCALE imbalance between joints, not DIFFICULTY
    imbalance, so a genuinely hard joint could still be systematically
    under-served when a shared upstream representation has to serve all
    22 joints' output layers at once.

    RESULT (real retrain, same data/subjects/protocol/seed as baseline):
    NO measurable effect. Fair R^2 0.695->0.693, pooled R^2 0.608->0.604
    (both within noise), and joint 2's own R^2 was EXACTLY IDENTICAL
    (0.519->0.519) to three decimal places -- explicitly and adaptively
    upweighting its loss whenever it fell behind changed nothing about
    its accuracy. This is a clean disproof, not just a null result: the
    hypothesis that joint 2's weakness is a gradient-priority/capacity-
    competition artifact is wrong. Its weakness is most likely a genuine
    predictability ceiling given the EMG signal itself for that joint,
    which loss reweighting cannot fix. Don't re-attempt per-joint loss
    reweighting on this architecture without new evidence pointing at a
    different cause."""
    def __init__(self, n_joints, decay=0.98, clip=(0.5, 2.5)):
        self.ema = torch.ones(n_joints)
        self.decay = decay
        self.clip = clip

    def __call__(self, out, yb):
        per_joint = ((out - yb) ** 2).mean(dim=0)   # (N_KIN,), keeps gradient
        with torch.no_grad():
            self.ema = self.ema.to(per_joint.device)
            self.ema = self.decay * self.ema + (1 - self.decay) * per_joint
            w = (self.ema / self.ema.mean()).clamp(*self.clip)
        return (w * per_joint).mean()
...
def main(data_dir="data", save_model=None, subject_ids=None):
    """subject_ids, if given, restricts training/eval to only files from
    that set of subject numbers -- used by the subject-pool-size
    sensitivity ablation (see subject_count_ablation.py) to test how much
    of this project's 19-subject pool (vs. sMAPEN's 40) plausibly
    inflates the reported R^2, without needing to acquire more DB2
    subjects' data."""
    files = sorted(glob.glob(os.path.join(data_dir, "*.mat")))[:qk.MAX_FILES]
    if not files:
        sys.exit(f"No .mat files in '{data_dir}'.")
    if subject_ids is not None:
        files = [f for f in files if qk.subject_of(f) in subject_ids]
        if not files:
            sys.exit(f"No files in '{data_dir}' match subject_ids={sorted(subject_ids)}.")
    ...
    lf = nn.MSELoss()   # plain MSE -- WeightedMSE (per-joint adaptive weighting) was tried and
                        # reverted here, see its docstring above for the real (null) result
